...
function replaceImageUrls(content, images) {
  ...
  for (const { url, fallbackUrl, filename } of images) {
    ...
    const escapedFallbackUrl = fallbackUrl.replace('/', '\\/')
    const regex = new RegExp(`^.*(?:${escapedUrl}|${escapedFallbackUrl}).*(?:alt="([^"]+?)").*$`, 'gm')
    result = result.replace(regex, `![$1](${fileRelPath})`);
    const regex2 = new RegExp(`^.*(?:${escapedUrl}|${escapedFallbackUrl}).*$`, 'gm')
    result = result.replace(regex2, `![](${fileRelPath})`);
  }
  return result;
}
...
function transformPost({
  ...
  const tags = [...new Set(tagnames?.split(',').map(x => x.trim()))];
  ...
  const convertedToHttps = imageReplaced?.split('http://mseeeen').join('https://mseeeen');
  const body = convertedToHttps?.split('https://mseeeen.msen.jp/').join('/');
...
`);
    ...
    console.log(`ERROR on write`, filePath, error);
  } finally {
    file?.close();
  }
...
async function processPost({ github, date, title, dirPath, tags, body, images, eyecatch }, { imageDownload }) {
  ...
  try {
    ...
    console.log(`ERROR on mkdir`, dirPath, error);
  }
  ...
  if (imageDownload) {
    ...
    try {
      ...
      console.log(`ERROR on mkdir (images)`, dirPath);
      throw error;
    }
    if (images?.length) {
